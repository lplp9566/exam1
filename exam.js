let militaryUnit = {
  ...
  const Mission1 =(unit)=>{
    const chiefOfStaff1 = unit.commandStructure.commandingOfficer
    return `${chiefOfStaff1.rank} ${chiefOfStaff1.name} ${chiefOfStaff1.contact.phone}`
  }
 

  const Mission2 = (unit)=>{
    res = 0
     unit.personnel.forEach((sol)=>{res+=1})
     return res
  }
  function Mission3 (NewDeployment , unit){
    ...
    const newhistry = {
        location:unit.currentDeployment.location,
        mission:unit.currentDeployment.mission,
        startDate:unit.currentDeployment.startDate,
        estimatedEndDate:unit.currentDeployment.estimatedEndDate

    }
    history.push(newhistry)
...
function Mission4( FirearmObject,unit){
    for (i = 0 ; i<unit.firearms.length;){
        if 
    }
